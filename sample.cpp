...
const char *WINDOWTITLE = { "CS553 Project 9 -- William Leslie" };
...
float TempLowHigh[2] = { 0, 100 };
GLUI_HSlider * TempSlider;
float opacity[2] = { 1, 1 };
GLUI_HSlider * OpacSlider;
...
GLUI_Spinner * PlaneZ;
float Zvalue;
GLUI_Spinner * isosurface;
float isoTemp = 50;
int iso;
const float GRAYMIN = .2;
const float GRAYMAX = 1;
/*
const float X[4] = { 1, -1, 0, 0 };
const float Y[4] = { 0, .3, 1, .4 };
const float Z[4] = { 0, 0, 0, 1 };
const float A[4] = { 90, 120, 120, 170 };
*/
...
float random(float max){
	return max * (-1 + 2* ((float)rand() / (RAND_MAX + 1)));
}
	...
	for (int zz = 0; zz < NY; zz++)
	{
		...
		for (int z = 0; z < NZ; z++)
		{
			...
			for (int x = 0; x < NX; x++)
			{
				...
				TextureXZ[zz][z][x][0] = (unsigned char)(255.*r + .5);
				TextureXZ[zz][z][x][1] = (unsigned char)(255.*g + .5);
				TextureXZ[zz][z][x][2] = (unsigned char)(255.*b + .5);
				TextureXZ[zz][z][x][3] = (unsigned char)(255.*alpha + .5);
			}
		}
	}
}
	...
	for (int zz = 0; zz < NX; zz++)
	{
		...
		for (int y = 0; y < NY; y++)
		{
			...
			for (int z = 0; z < NZ; z++)
			{
				...
				TextureYZ[zz][z][y][0] = (unsigned char)(255.*r + .5);
				TextureYZ[zz][z][y][1] = (unsigned char)(255.*g + .5);
				TextureYZ[zz][z][y][2] = (unsigned char)(255.*b + .5);
				TextureYZ[zz][z][y][3] = (unsigned char)(255.*alpha + .5);
			}
		}
	}
}
...
void
ProcessQuad(float temp, struct node *p0, struct node *p1, struct node *p2, struct node *p3)
{
	float tstar1, tstar2, tstar3, tstar4;
	float x1, x2, x3, x4;
	float y1, y2, y3, y4;
	float z1, z2, z3, z4;
	int count= 0;
	tstar1 = (temp - p0->T)/(p1->T - p0->T);
	tstar2 = (temp - p1->T) / (p2->T - p1->T);
	tstar3 = (temp - p2->T) / (p3->T - p2->T);
	tstar4 = (temp - p3->T) / (p0->T - p3->T);
	x1 = p0->x + tstar1*(p1->x - p0->x);
	y1 = p0->y + tstar1*(p1->y - p0->y);
	z1 = p0->z + tstar1*(p1->z - p0->z);
	x2 = p1->x + tstar2*(p2->x - p1->x);
	y2 = p1->y + tstar2*(p2->y - p1->y);
	z2 = p1->z + tstar2*(p2->z - p1->z);
	x3 = p2->x + tstar3*(p3->x - p2->x);
	y3 = p2->y + tstar3*(p3->y - p2->y);
	z3 = p2->z + tstar3*(p3->z - p2->z);
	x4 = p3->x + tstar4*(p0->x - p3->x);
	y4 = p3->y + tstar4*(p0->y - p3->y);
	z4 = p3->z + tstar4*(p0->z - p3->z);
	if (tstar1 >= 0 && tstar1 <= 1){
		count++;
	}
	if (tstar2 >= 0 && tstar2 <= 1){
		count++;
	}
	if (tstar3 >= 0 && tstar3 <= 1){
		count++;
	}
	if (tstar4 >= 0 && tstar4 <= 1){
		count++;
	}
	if (count != 2){
		return;
...
Display(void)
{
	...
	glEnable(GL_BLEND);
	float z0, dz, x0, dx, y0, dy, zcoord, xcoord, ycoord;
	int z, y, x;
	if (Major == Z)
	{
		if (Zside == PLUS)
	...
	if (Major == X)
	{
		...
		{
			glTexImage2D(GL_TEXTURE_2D, 0, 4, NX, NY, 0, GL_RGBA, GL_UNSIGNED_BYTE, &TextureYZ[x][0][0][0]);
	...
	if (Major == Y)
	{
		...
		{
			glTexImage2D(GL_TEXTURE_2D, 0, 4, NX, NY, 0, GL_RGBA, GL_UNSIGNED_BYTE, &TextureXZ[y][0][0][0]);
	...
	//				PlaneXY[i][j].x = x;
	//				PlaneXY[i][j].y = y;
	//				PlaneXY[i][j].z = Zvalue;
	//				PlaneXY[i][j].T = Temperature(x, y, Zvalue);
	...
	//					glBegin(GL_QUADS);
	//					glColor3f(PlaneXY[i][j].r, PlaneXY[i][j].g, PlaneXY[i][j].b);
	//					glVertex3f(PlaneXY[i][j].x, PlaneXY[i][j].y, PlaneXY[i][j].z);
	//					glColor3f(PlaneXY[i + 1][j].r, PlaneXY[i + 1][j].g, PlaneXY[i + 1][j].b);
	//					glVertex3f(PlaneXY[i + 1][j].x, PlaneXY[i + 1][j].y, PlaneXY[i + 1][j].z);
	//					glColor3f(PlaneXY[i + 1][j + 1].r, PlaneXY[i + 1][j + 1].g, PlaneXY[i + 1][j + 1].b);
	//					glVertex3f(PlaneXY[i + 1][j + 1].x, PlaneXY[i + 1][j + 1].y, PlaneXY[i + 1][j + 1].z);
	//					glColor3f(PlaneXY[i][j + 1].r, PlaneXY[i][j + 1].g, PlaneXY[i][j + 1].b);
	//					glVertex3f(PlaneXY[i][j + 1].x, PlaneXY[i][j + 1].y, PlaneXY[i][j + 1].z);
	//					glEnd();
	//				}
	//			}
	//		}
	//		if (contours == 1){
	//			for (float Sstar = Smin; Sstar <= Smax; Sstar += 10){
	...
	//				for (int i = 0; i < NX - 1; i++){
	//					for (int j = 0; j < NY - 1; j++){
	//						ProcessQuad(Sstar, &PlaneXY[i][j], &PlaneXY[i + 1][j], &PlaneXY[i + 1][j + 1], &PlaneXY[i][j + 1]);
	//					}
	//				}
	//				glEnd();
	//			}
	...
	//		for (int i = 0; i < NY; i++)
	//		{
	//			float y = -1. + 2. * (float)i / (float)(NY - 1);
	//			for (int j = 0; j < NZ; j++)
	//			{
	//				float z = -1. + 2. * (float)j / (float)(NZ - 1);
	//				PlaneYZ[i][j].x = Xvalue;
	//				PlaneYZ[i][j].y = y;
	//				PlaneYZ[i][j].z = z;
	//				PlaneYZ[i][j].T = Temperature(Xvalue, y, z);
	...
	//					glShadeModel(GL_SMOOTH);
	//					glBegin(GL_QUADS);
	//					glColor3f(PlaneYZ[i][j].r, PlaneYZ[i][j].g, PlaneYZ[i][j].b);
	//					glVertex3f(PlaneYZ[i][j].x, PlaneYZ[i][j].y, PlaneYZ[i][j].z);
	//					glColor3f(PlaneYZ[i + 1][j].r, PlaneYZ[i + 1][j].g, PlaneYZ[i + 1][j].b);
	//					glVertex3f(PlaneYZ[i + 1][j].x, PlaneYZ[i + 1][j].y, PlaneYZ[i + 1][j].z);
	//					glColor3f(PlaneYZ[i + 1][j + 1].r, PlaneYZ[i + 1][j + 1].g, PlaneYZ[i + 1][j + 1].b);
	//					glVertex3f(PlaneYZ[i + 1][j + 1].x, PlaneYZ[i + 1][j + 1].y, PlaneYZ[i + 1][j + 1].z);
	//					glColor3f(PlaneYZ[i][j + 1].r, PlaneYZ[i][j + 1].g, PlaneYZ[i][j + 1].b);
	//					glVertex3f(PlaneYZ[i][j + 1].x, PlaneYZ[i][j + 1].y, PlaneYZ[i][j + 1].z);
	//					glEnd();
	//				}
	//			}
	//		}
	...
	//						ProcessQuad(Sstar, &PlaneYZ[i][j], &PlaneYZ[i + 1][j], &PlaneYZ[i + 1][j + 1], &PlaneYZ[i][j + 1]);
	//					}
	//				}
	//				glEnd();
	//			}
	...
	//				float z = -1. + 2. * (float)j / (float)(NZ - 1);
	//				PlaneXZ[i][j].x = x;
	//				PlaneXZ[i][j].y = Yvalue;
	//				PlaneXZ[i][j].z = z;
	//				PlaneXZ[i][j].T = Temperature(x, Yvalue, z);
	...
	//					glShadeModel(GL_SMOOTH);
	//					glBegin(GL_QUADS);
	//					glColor3f(PlaneXZ[i][j].r, PlaneXZ[i][j].g, PlaneXZ[i][j].b);
	//					glVertex3f(PlaneXZ[i][j].x, PlaneXZ[i][j].y, PlaneXZ[i][j].z);
	//					glColor3f(PlaneXZ[i + 1][j].r, PlaneXZ[i + 1][j].g, PlaneXZ[i + 1][j].b);
	//					glVertex3f(PlaneXZ[i + 1][j].x, PlaneXZ[i + 1][j].y, PlaneXZ[i + 1][j].z);
	//					glColor3f(PlaneXZ[i + 1][j + 1].r, PlaneXZ[i + 1][j + 1].g, PlaneXZ[i + 1][j + 1].b);
	//					glVertex3f(PlaneXZ[i + 1][j + 1].x, PlaneXZ[i + 1][j + 1].y, PlaneXZ[i + 1][j + 1].z);
	//					glColor3f(PlaneXZ[i][j + 1].r, PlaneXZ[i][j + 1].g, PlaneXZ[i][j + 1].b);
	//					glVertex3f(PlaneXZ[i][j + 1].x, PlaneXZ[i][j + 1].y, PlaneXZ[i][j + 1].z);
	//					glEnd();
	//				}
	//			}
	//		}
	...
	//				for (int i = 0; i < NX - 1; i++){
	//					for (int j = 0; j < NZ - 1; j++){
	//						ProcessQuad(Sstar, &PlaneXZ[i][j], &PlaneXZ[i + 1][j], &PlaneXZ[i + 1][j + 1], &PlaneXZ[i][j + 1]);
	//					}
	//				}
	//				glEnd();
	//			}
	...
	//else{
	//	float hsv[3], rgb[3];
	//	hsv[0] = 240. - 240.*(isoTemp - TEMPMIN) / (TEMPMAX - TEMPMIN);
	//	hsv[1] = 1.;
	//	hsv[2] = 1.;
	//	HsvRgb(hsv, rgb);
	//	glColor3f(rgb[0], rgb[1], rgb[2]);
	...
	//				PlaneYZ[j][k].y = y;
	//				PlaneYZ[j][k].z = z;
	//				PlaneYZ[j][k].T = Temperature(x, y, z);
	//			}
	//		}
	//		glBegin(GL_LINES);
	//		for (int j = 0; j < NY - 1; j++){
	//			for (int k = 0; k < NZ - 1; k++){
	//				ProcessQuad(isoTemp, &PlaneYZ[j][k], &PlaneYZ[j + 1][k], &PlaneYZ[j + 1][k + 1], &PlaneYZ[j][k + 1]);
	//			}
	//		}
	//		glEnd();
	...
	//				PlaneXZ[i][k].y = y;
	//				PlaneXZ[i][k].z = z;
	//				PlaneXZ[i][k].T = Temperature(x, y, z);
	//			}
	//		}
	//		glBegin(GL_LINES);
	//		for (int i = 0; i < NX - 1; i++){
	//			for (int k = 0; k < NZ - 1; k++){
	//				ProcessQuad(isoTemp, &PlaneXZ[i][k], &PlaneXZ[i + 1][k], &PlaneXZ[i + 1][k + 1], &PlaneXZ[i][k + 1]);
	//			}
	//		}
	//		glEnd();
	...
	//				PlaneXY[i][j].y = y;
	//				PlaneXY[i][j].z = z;
	//				PlaneXY[i][j].T = Temperature(x, y, z);
	//			}
	//		}
	//		glBegin(GL_LINES);
	//		for (int i = 0; i < NX - 1; i++){
	//			for (int j = 0; j < NY - 1; j++){
	//				ProcessQuad(isoTemp, &PlaneXY[i][j], &PlaneXY[i + 1][j], &PlaneXY[i + 1][j + 1], &PlaneXY[i][j + 1]);
	//			}
	//		}
	//		glEnd();
...
InitGlui( void )
{
	...
	//Glui->add_checkbox("Jitter", &JitterOn);
	//Glui->add_checkbox("Large Points", &BigPoints);
	//Glui->add_checkbox("XY Plane", &XY);
	//Glui->add_checkbox("YZ Plane", &YZ);
	//Glui->add_checkbox("XZ Plane", &XZ);
	//Glui->add_checkbox("Contours", &contours);
	//Glui->add_checkbox("Isosurface", &iso);
	...
	OpacSlider = Glui->add_slider(false, GLUI_HSLIDER_FLOAT, opacity,
		TEMP, (GLUI_Update_CB)Sliders);
	OpacSlider->set_float_limits(0, 1);
	OpacSlider->set_w(200);		// good slider width
	/*
	RadiusSlider = Glui->add_slider(true, GLUI_HSLIDER_FLOAT, RadiusLowHigh,
		RAD, (GLUI_Update_CB)Sliders);
	RadiusSlider->set_float_limits(0, sqrt(3.));
	RadiusSlider->set_w(200);		// good slider width
	sprintf(str, RADFORMAT, RadiusLowHigh[0], RadiusLowHigh[1]);
	RadiusLabel = Glui->add_statictext(str);
	
	GradientSlider = Glui->add_slider(true, GLUI_HSLIDER_FLOAT, GradientLowHigh,
		GRAD, (GLUI_Update_CB)Sliders);
	GradientSlider->set_float_limits(0, 300);
	GradientSlider->set_w(200);		// good slider width
	sprintf(str, GRADFORMAT, GradientLowHigh[0], GradientLowHigh[1]);
	GradientLabel = Glui->add_statictext(str);
	
	XSlider = Glui->add_slider(true, GLUI_HSLIDER_FLOAT, XLowHigh,
		TEMP, (GLUI_Update_CB)Sliders);
	...
	YSlider = Glui->add_slider(true, GLUI_HSLIDER_FLOAT, YLowHigh,
		TEMP, (GLUI_Update_CB)Sliders);
	...
	ZSlider = Glui->add_slider(true, GLUI_HSLIDER_FLOAT, ZLowHigh,
		TEMP, (GLUI_Update_CB)Sliders);
	...
	PlaneZ = Glui->add_spinner("XY",
		GLUI_SPINNER_FLOAT,
		&Zvalue, 0);

	PlaneX = Glui->add_spinner("YZ",
		GLUI_SPINNER_FLOAT,
		&Xvalue, 1);

	PlaneY = Glui->add_spinner("XZ",
		GLUI_SPINNER_FLOAT,
		&Yvalue, 2);

	isosurface = Glui->add_spinner("Temp for isosurface",
		GLUI_SPINNER_FLOAT,
		&isoTemp, 3);
		*/
...
InitGraphics( void )
{
	...
	TempLowHigh[1] = 100;
	RadiusLowHigh[0] = 0;
	RadiusLowHigh[1] = sqrt(3.);
	GradientLowHigh[0] = 0;
	GradientLowHigh[1] = 300;
	XLowHigh[0] = -1;
	XLowHigh[1] = 1;
	YLowHigh[0] = -1;
	YLowHigh[1] = 1;
	...
	for (int i = 0; i < NX; i++)
	{
		...
		for (int j = 0; j < NY; j++)
		{
			...
			for (int k = 0; k < NZ; k++)
			{
				...
				Nodes[i][j][k].g = rgb[1];
				Nodes[i][j][k].b = rgb[2];
				Nodes[i][j][k].rad = sqrt(SQR(x) + SQR(y) + SQR(z));
	...
	for (int i = 0; i < NX; i++)
	{
		...
		for (int j = 0; j < NY; j++)
		{
			...
			for (int k = 0; k < NZ; k++)
			{
				float z = -1. + 2. * (float)k / (float)(NZ - 1);
				if (i == 0){
					Nodes[i][j][k].dTdx = (Nodes[i + 1][j][k].T) / (Nodes[i + 1][j][k].x);
				}
				else if (i == NX - 1){
					Nodes[i][j][k].dTdx = (Nodes[i - 1][j][k].T) / (Nodes[i - 1][j][k].x);
				}
				...
				if (j == 0){
					Nodes[i][j][k].dTdy = (Nodes[i][j + 1][k].T / (Nodes[i][j + 1][k].y));
				}
				else if (j == NY - 1){
					Nodes[i][j][k].dTdy = (Nodes[i][j - 1][k].T) / (Nodes[i][j - 1][k].y);
				}
				...
				if (j == 0){
					Nodes[i][j][k].dTdz = (Nodes[i][j][k + 1].T / (Nodes[i][j][k + 1].z));
				}
				else if (j == NZ - 1){
					Nodes[i][j][k].dTdz = (Nodes[i][j][k - 1].T) / (Nodes[i][j][k - 1].z);
				}
...
InitLists( void )
{
	...
	for (int i = 0; i < NX; i++)
	{
		...
		for (int j = 0; j < NY; j++)
		{
			float y = -1. + 2. * (float)j / (float)(NY - 1);
				...
				{
					float z = -1. + 2. * (float)k / (float)(NZ - 1);
					shade = setGray(Temperature(x, y, z));
	...
	glEnd();
	glEndList();
	BigList = glGenLists(1);
	glNewList(BigList, GL_COMPILE);
	glPointSize(2);
	if (BigPoints != 0){
		glPointSize(2);
	}
	...
	glNewList( BoxList, GL_COMPILE );
		...
		glEnd( );
	JitteredList = glGenLists(1);
	glNewList(JitteredList, GL_COMPILE);
	glPointSize(1);
	if (BigPoints != 0){
		glPointSize(2);
	}
	...
	for (int i = 0; i < NX; i++)
	{
		float x = -1. + 2. * (float)i / (float)(NX - 1)+random(.01);
		for (int j = 0; j < NY; j++)
		{
			float y = -1. + 2. * (float)j / (float)(NY - 1)+random(.01);
			for (int k = 0; k < NZ; k++)
			{
				float z = -1. + 2. * (float)k / (float)(NZ - 1)+random(.01);
				shade = setGray(Temperature(x, y, z));
				glColor3f(shade, shade, shade);
				glVertex3f(x, y, z);
			}
	...
	glEnd();
	glEndList( );
	JitteredBigList = glGenLists(1);
	glNewList(JitteredBigList, GL_COMPILE);
	glPointSize(2);
	if (BigPoints != 0){
		glPointSize(2);
	}
